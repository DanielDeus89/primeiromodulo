const lessonTitle = "Lesson 09";
const currentVideoId = "M1IDJouaCwk";

const timeRanges = [
  { start: 9, end: 286 },
  { start: 296, end: 502 }, //https://youtu.be/qVWg4NkpmDA?t=89
  { start: 502, end: 9999 },   //https://youtu.be/qVWg4NkpmDA?t=109
  { start: 110, end: 99999 }, // https://youtu.be/qVWg4NkpmDA?t=175
  { start: 179, end: 99999 }, // https://youtu.be/qVWg4NkpmDA?t=233
  { start: 233, end: 99999 }, // https://youtu.be/qVWg4NkpmDA?t=291
  { start: 291, end: 99999 }, // https://youtu.be/qVWg4NkpmDA?t=332
  { start: 332, end: 99999 }, // https://youtu.be/qVWg4NkpmDA?t=369 https://youtu.be/qVWg4NkpmDA?t=333
  { start: 369, end: 99999 }, //https://youtu.be/qVWg4NkpmDA?t=599
  { start: 605, end: 99999 }, //https://youtu.be/qVWg4NkpmDA?t=935
  { start: 851, end: 99999 }, //https://youtu.be/qVWg4NkpmDA?t=851
  { start: 941, end: 99999 }, //https://youtu.be/qVWg4NkpmDA?t=1184
];

const lessonCards = [
  {
  "title": "Input 09",
  "columns": [
    [
      ["I have.", "Eu tenho."],
      ["I have a cat.", "Eu tenho um gato."],
      ["You have a dog.", "Você tem um cachorro."],
      ["I have a bicycle.", "Eu tenho uma bicicleta."],
      ["You have a table.", "Você tem uma mesa."],
      ["I have time.", "Eu tenho tempo."]
    ],
    [
      ["I understand.", "Eu entendo."],
      ["I understand Spanish.", "Eu entendo espanhol."],
      ["You understand German.", "Você entende alemão."],
      ["I understand Chinese.", "Eu entendo chinês."],
      ["You understand French.", "Você entende francês."],
      ["I understand English.", "Eu entendo inglês."]
    ]
  ]
},
{
  "title": "Input 09",
  "columns": [
    [
      ["I have to study.", "Eu tenho que estudar."],
      ["You have to sleep.", "Você tem que dormir."],
      ["I have to work.", "Eu tenho que trabalhar."],
      ["You have to eat.", "Você tem que comer."]
    ],
    [
      ["I want to understand.", "Eu quero entender."],
      ["I have to understand Chinese.", "Eu tenho que entender chinês."],
      ["You understand me.", "Você me entende."],
      ["You have to understand your husband.", "Você tem que entender seu marido."]
    ]
  ]
},
{
  "title": "Input 09",
  "columns": [
    [
      ["Where do you work?", "Onde você trabalha?"],
      ["Where do you study?", "Onde você estuda?"],
      ["Where do you play?", "Onde você joga?"],
      ["Where do you eat?", "Onde você come?"],
      ["Where do you sleep?", "Onde você dorme?"]
    ],
    [
      ["Where do you have to go?", "Onde você tem que ir?"],
      ["Where do you want to study?", "Onde você quer estudar?"],
      ["Where do you want to play?", "Onde você quer jogar?"],
      ["Where do you want to work?", "Onde você quer trabalhar?"],
      ["Where do you like to go on weekends?", "Onde você gosta de ir nos fins de semana?"]
    ]
  ]
},
{
  "title": "Verbs",
  "columns": [
    [["to have", "ter"]],
    [["to understand", "entender"]]
  ]
},
{
  "title": "Vocabulary",
  "columns": [
    [
      ["cat", "gato"],
      ["dog", "cachorro"],
      ["bicycle", "bicicleta"],
      ["table", "mesa"],
      ["every", "todo, cada"],
      ["too", "também"],
      ["also", "também"]
    ],
    [
      ["where", "onde, aonde"],
      ["time", "tempo"],
      ["some", "algum"],
      ["German", "alemão"],
      ["Chinese", "chinês"],
      ["weekend", "fim de semana"]
    ]
  ]
},
{
  "title": "Expressions",
  "columns": [
    [
      ["see you", "até logo"],
      ["see you later", "até logo"]
    ],
    [
      ["see you tomorrow", "até amanhã"],
      ["on weekends", "nos fins de semana"]
    ],
    [
      ["email account", "conta de email"]
    ]
  ]
},
{
  "title": "Grammar",
  "columns": [
    [
      ["Where do you work?", "Onde você trabalha?"],
      ["I have to...", "Eu tenho que..."],
      ["I have to study.", "Eu tenho que estudar."],
      ["I have to work.", "Eu tenho que trabalhar."]
    ],
    [
      ["I have a car.", "Eu tenho um carro."],
      ["I have to go now.", "Eu tenho que ir agora."]
    ]
  ]
}];